...
def _validate_and_convert_tensors(checkpoint) -> dict[str, torch.Tensor]:
    ...
    if missing or unexpected:
        raise ValueError(
            "MiniMax-H3 Turbo target set does not match the supported 4-step artifacts: "
...
def load_minimax_h3_turbo_lora(
    *,
    ...
    """Load LightX2V Turbo 4-step v1.0/v1.2 through the legacy manager."""
    ...
    with safe_open(lora_file, framework="pt", device="cpu") as checkpoint:
        ...
        if metadata.get("key_format") != "minimax-h3-diffusers":
            if lora_file.name in _TURBO_ARTIFACT_ALPHAS:
                raise ValueError(
                    "MiniMax-H3 Turbo requires safetensors metadata key_format='minimax-h3-diffusers'"
                )
            return None
        if lora_file.name not in _TURBO_ARTIFACT_ALPHAS:
            raise ValueError(
                f"MiniMax-H3 Turbo supports only {tuple(_TURBO_ARTIFACT_ALPHAS)!r}, got {lora_file.name!r}"
            )
        expected_alpha = _TURBO_ARTIFACT_ALPHAS[lora_file.name]
        ...
        if alpha != expected_alpha:
            raise ValueError(
                f"MiniMax-H3 Turbo {lora_file.name} requires alpha={expected_alpha}, got {raw_alpha!r}"
            )
        if partition == "ref2va":
