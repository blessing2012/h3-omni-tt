...
@pytest.mark.parametrize(("version", "alpha"), [("1.0", 128), ("1.2", 8)])
def test_h3_turbo_loads_through_legacy_lora_model_and_swaps_ffn(tmp_path, version, alpha):
    path = tmp_path / f"minimax_h3_fl2v_turbo_4step_v{version}_768p_bf16.safetensors"
    _write_tiny_turbo(path, alpha=str(alpha))
    ...
    assert peft_helper.lora_alpha == alpha
    ...
    # Check the effective update, including alpha/rank, for both ordinary and
    # packed projections. v1.2 must not inherit v1.0's 16x larger scaling.
    q = lora_model.get_lora("blocks.0.attn.to_q")
    torch.testing.assert_close(q.lora_b @ q.lora_a * q.scaling, torch.full((3, 2), float(alpha)))
    for index, multiplier in enumerate((2, 1)):
        torch.testing.assert_close(
            fc1.lora_b[index] @ fc1.lora_a[index] * fc1.scaling[index],
            torch.full((2, 2), float(multiplier * alpha)),
        )
...
@pytest.mark.parametrize(("version", "alpha"), [("1.0", 128), ("1.2", 8)])
def test_legacy_manager_uses_the_h3_model_loader_without_changing_its_interface(tmp_path, version, alpha):
    path = tmp_path / f"minimax_h3_fl2v_turbo_4step_v{version}_768p_bf16.safetensors"
    _write_tiny_turbo(path, alpha=str(alpha))
    ...
    # The manager folds alpha/rank into B exactly once before binding layers.
    q = lora_model.get_lora("blocks.0.attn.to_q")
    fc1 = lora_model.get_lora("blocks.0.mlp.fc1")
    assert q.scaling == 1
    assert fc1.scaling == [1, 1]
    for _ in range(2):
        q.optimize()
        fc1.optimize()
        torch.testing.assert_close(q.lora_b @ q.lora_a, torch.full((3, 2), float(alpha)))
        for index, multiplier in enumerate((2, 1)):
            torch.testing.assert_close(
                fc1.lora_b[index] @ fc1.lora_a[index],
                torch.full((2, 2), float(multiplier * alpha)),
            )


@pytest.mark.parametrize(("version", "alpha", "bad_alpha"), [("1.0", 128, 8), ("1.2", 8, 128)])
def test_h3_turbo_rejects_wrong_alpha_and_ref2va(tmp_path, version, alpha, bad_alpha):
    filename = f"minimax_h3_fl2v_turbo_4step_v{version}_768p_bf16.safetensors"
    wrong_alpha = tmp_path / "wrong_alpha" / filename
    wrong_alpha.parent.mkdir()
    _write_tiny_turbo(wrong_alpha, alpha=str(bad_alpha))
    with pytest.raises(ValueError, match=f"requires alpha={alpha}"):
        load_minimax_h3_turbo_lora(
            partition="fl2va",
    ...
    valid = tmp_path / "valid" / filename
    valid.parent.mkdir()
    _write_tiny_turbo(valid, alpha=str(alpha))
